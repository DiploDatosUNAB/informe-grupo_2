...
El promedio general de precios es de `r round(vinos$precio |> mean(na.rm = T), 2)` euros.  
Su desvío estándar `r round(vinos$precio |> sd(na.rm = T), 2)`.  
...
El promedio general de puntos es de `r round(vinos$puntos |> mean(na.rm = T))` puntos.  
Su desvío estándar `r round(vinos$puntos |> sd(na.rm = T))`.  
...
vinos |> 
  ...
  geom_point(aes(color = puntos), alpha = 1/20, na.rm = T) +
